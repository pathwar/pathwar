...
func TestService_UserGetSession(t *testing.T) {
	...
	var session *UserGetSession_Output
	{
		...
		assert.Equal(t, session.Claims.AccessToken.Sub, pwsso.TestingClaims(t).AccessToken.Sub)
	...
	// login
	{
		...
		assert.Equal(t, session2.Claims.AccessToken.Sub, pwsso.TestingClaims(t).GetAccessToken().Sub)
